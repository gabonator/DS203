...
{
	ui8* pCommonBuffer = (ui8*)&BIOS::ADC::GetAt(0);
	BIOS::FAT::TFindFile* arrFiles = (BIOS::FAT::TFindFile*)pCommonBuffer;
	int nDataLength = BIOS::ADC::BufferLength;
...
void CWndUserManager::DrawLine( BIOS::FAT::TFindFile& fileInfo, int y, bool bSelected )
{
	...
	{
		BIOS::DBG::sprintf( strAux, "%02d/%02d/%02d", (fileInfo.nDate>>5)&15, (fileInfo.nDate)&31, (fileInfo.nDate>>9)-20);
...
void CWndUserManager::Exec(char* strPath, char* strFile, int nLength)
{
	...
	if ( eType == ENone )
	{
		MainWnd.m_wndMessage.Show(this, "Manager", "Unknown file suffix", RGB565(FF0000));
		return;
	}
	...
	if ( eType == EAdr )
	{
		...
		{
			MainWnd.m_wndMessage.Show(this, "Manager", "Image flashed successfully", RGB565(FFFF00));
